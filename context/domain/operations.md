...
| `offers.ts` | **what the client received** (pure, client-safe; also the `OfferDialogDeal` type): `OFFER_CONTENTS` / `OFFER_CONTENT_LABEL`, `parseOfferMeta`, `offerSummary`, `offerNote`, `lastOfferOf` (the last valid send, shown under "Naposledy"), `moneyToString`, `formatMoney`, `offerInstant`, `compareOffers` (order: `sentOn`, historical before normal on the same day, then `createdAt`), `summarizeOffers(rows)` (first about-us / cenník, last price + snapshot, first send per design; valid rows only), `clientKnowledge` (yes / "?" / no — legacy fields never mean yes), `legacyUnreviewed`, `isValidSentOn`. |
| `offerMutations.ts` | `recomputeOffers(tx, leadId)` — the **only** writer of `Lead.offer*`, `Lead.designSentAt`, `Design.sentAt`; `recordOffer(tx, actor, lead, input, source)` — writes one `OFFER_SENT` (price snapshot; saves the lead's price first if given — a new amount without a breakdown clears the old breakdown, the same amount keeps it; a design marked sent by old code gets its date baselined into `legacySentAt` before the first new send), recomputes, optionally plans "Zavolať, či prišlo" in 7 days, and until wave 3 closes matching open requests (never for a historical entry); `correctRecord` — crosses out an `OFFER_SENT` / `SMS_SENT` / `CLIENT_REPLIED` (reason in `meta.correction`), recomputes; never touches the next step or requests; `CORRECTABLE_TYPES`. |
...
| `idempotency.ts` | `idempotentReplay(key, { userId, leadId, source, outcome })` → `null` if the key is new; success without another write if the same CALL was already recorded; `IDEMPOTENCY_CONFLICT` if the key belongs to a different write. For a handoff replay, the toast recipient is reconstructed from the lead's **current** owner, so it can differ after a later transfer; the original CALL author/history is unchanged. CALL activities only. `activityReplay(key, { userId, leadId, types, fingerprint?, want? })` — the same idea for any other activity type (OFFER_SENT, CLIENT_REPLIED, SMS_SENT, the planning row of "bez kontaktu"); with a fingerprint, the same key carrying **different content** is `IDEMPOTENCY_CONFLICT`, never a false success (`offerFingerprint` / `offerFingerprintOfMeta` in `offers.ts`). |
...
| `logFollowUpAs(user, input)` (`dealWork.ts`) | the interaction: strict input, `requireDealWork` + revision + idempotency. `contact` decides the history row: `CALL` → `CALL`, `REPLIED` → `CLIENT_REPLIED`, `SMS` → `SMS_SENT` (note only; outcome must be `POSITIVE`), `NONE` → no contact row (outcome `POSITIVE`, the key sits on the planning row). Applies `dealStateForFollowUp` (for `NONE` the deal keeps its status — a snoozed deal stays snoozed), plans the next step, opens a request when the outcome implies one, closes requests when the deal closes. `phonePrice` (CALL only, not with no-answer / closing outcomes) writes an `OFFER_SENT` channel `PHONE` linked to the call in the same transaction. `sourceFor(user)` is exported. |
| `updateDealContactAs`, `saveDealQuoteAs` (`dealWork.ts`) | owner-or-manager deal work, open deals only for the owner. Activity source follows the actor: `deals.manage` → `PIPELINE`, else `CLIENTS`. |
| `setDealNextActionAs`, `addDealNoteAs` (`dealWork.ts`) | **no UI caller since wave 3b** (the detail changes the step through the action sheet, "bez kontaktu"; free notes were removed). Kept, used by the check scripts. |
| `recordOfferSentAs(user, input)` (`offers.ts`) | "Čo sme poslali": strict input (`contents`, `sentOn` ≤ today, optional `price`, `designIds`, `followUp` + optional `followUpOn` day ≥ today, `historical`), `requireDealWork` (manager may act on a closed deal) + `expectedRevision` + `idempotencyKey` (`activityReplay`). `historical` = manager only, only on an unreviewed legacy deal, never `followUp`. |
...
| `updateLeadAs`, `saveQuoteAs`, `setProjectTypeAs`, `markLostAs`, `changeStatusAs`, `reopenDealAs`, `changeOwnerAs` (`pipeline.ts`) | manager-only (`requireDealManage`), any deal state. `setNextActionAs` and `addBusinessNoteAs` (`NOTE` only) have **no UI caller since wave 3b**; kept for the check scripts. |
...
| `getDealList(params)` | the pipeline list (each row also carries `dialog` — the data "Čo sme poslali" needs, so it opens in place — and `lastOffer`): filters in Prisma (ids only), ordering + `LIMIT` in SQL over the whole filtered set (`DEAL_PAGE_SIZE = 50`), full rows for the page, `noAnswerStreak` per page in one window query (consecutive non-reverted `NO_ANSWER` since the last `CALL` / `CLIENT_REPLIED`). Row "Naposledy" = `LAST_TOUCH_WHERE`: only `LAST_TOUCH_TYPES` (real client contact — CALL, CLIENT_REPLIED, SMS_SENT, OFFER_SENT, NOTE and the legacy send types), not crossed out, not a phone price, not a historical send. The page fetch re-applies the scope. Views `got_pricelist` / `got_price` / `got_design` / `unverified` (legacy, cross-status). |
...
| Script | Purpose |
|---|---|
| `2026-09-offer-migrate.ts` | **prototype, not safe to apply** (`db-changes.md` §3.3): currently proposes old-send `OFFER_SENT` rows using a date cutoff for cenník and other unverified assumptions. Dry-run/report exists; its apply/verify guards are not sufficient for the chosen full migration. The target conversion is decided, but this script has not implemented it. |
